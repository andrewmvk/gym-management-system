...
2. **Training**: a human personal trainer verbally interviews the customer on the spot, decides a plan for the day, and manually records it. This doesn't scale, isn't personalized beyond that single conversation, isn't available 24/7, and loses historical nuance (injuries, medication, life events) between sessions.
...
- **24/7 personalization**: a customer can get plan feedback and adjustments any time, not just during a trainer's shift.
...
- **Community awareness**: customers and the gym can see aggregate, real-time-style info (occupancy, popular exercises/equipment) derived from other members' plans.
...
| Role | Who | Core need |
|---|---|---|
| **Customer** | Gym member | Sign up, get cleared to train, get a personalized daily plan, check in via face recognition, talk to an AI about their training/state, see their history and metrics |
| **Personal Trainer** | Gym staff | Oversee AI-generated plans across all customers (shared pool, not 1:1 assigned), comment on / override any plan |
...
- **No visible persistent chat log**: the AI chat is stateless in the UI, but every message is mined for structured facts that are saved permanently to the customer's profile/history — this is what makes future plans "remember" the customer.
