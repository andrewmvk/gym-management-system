...
## 1. Customer: Signup → Aptitude → Account Creation

```
1. Customer fills basic signup form: name, phone, email, birthdate.
2. Customer captures a reference photo via webcam. The backend computes a
...
3. Customer fills the digital aptitude/health questionnaire.
4. AI evaluates the questionnaire, returning cleared / not_cleared / pending_retry:
   a. cleared       → go to step 7.
   b. pending_retry  → customer sees "still processing, check back shortly";
                        system retries automatically.
   c. not_cleared    → customer must upload a medical certificate.
5. Customer uploads a medical certificate. AI reviews it, returning
...
6. Every certificate result — regardless of which of the three — is routed
   ...
   c. pending_retry stays queued until the AI resolves it or Admin
      manually decides.
7. Customer sets a password → account created, authenticated session begins.
...
- The Admin override in step 6a is the *only* recovery path for a customer the AI has wrongly rejected, since they have no account to log in and contest it themselves.

## 2. Customer: Detailed Onboarding (post-authentication)

```
1. Customer opens the onboarding link from their email — or, if they never
...
2. Customer submits: exam attachments, medications, physical
   conditions/limitations, goals/pretensions, and any other free-form info.
3. Onboarding data is saved to the customer's profile.
4. AI generates the customer's first training plan from this data.
5. Plan is published immediately (visible to customer right away).
6. Plan appears in the trainer's shared review queue for asynchronous oversight.
```

Onboarding is not a one-time gate — customers can add or revise this information later via the AI chat (flow 4), and each addition can trigger plan regeneration.

## 3. Customer: Facial Recognition Check-in (kiosk panel)

```
1. Customer approaches the gym panel (kiosk web app) and looks at the camera.
2. The kiosk holds only precomputed face embeddings for all registered
   customers, fetched from a dedicated, kiosk-authenticated backend
...
3. Panel captures a photo, computes its embedding client-side, and searches
   ...
   b. No candidate clears the threshold, or the top two are too close to
      call → rejected; customer is asked to retry.
4. On an accepted match:
   ...
   b. The check-in (customer, timestamp, turnstile_status: success|failed)
...
5. On no match: access denied, customer can retry or seek staff help.
```

## 4. Customer: Daily Plan, History, and AI Chat

```
1. Customer logs in, sees today's plan (exercises from the curated library:
...
2. Customer can browse plan history by date.
3. Customer opens the AI chat (no persistent visible thread) and can:
...
4. Every message is parsed by the AI to extract structured facts, which are
   saved permanently to the customer's history — this is what the AI reads
...
5. If the requested adjustment targets a plan a trainer has already edited,
   the customer is first shown a warning ("A trainer already adjusted this
...
```
1. Customer opens their metrics view.
...
1. Trainer logs in (via a seeded demo account — see flow 7), sees a shared
   queue of all customers' current/recent AI-generated plans (no 1:1
   assignment — any trainer can act on any plan).
2. Trainer opens a customer's plan, can:
   ...
   - Edit the plan directly (exercises, sets/reps) — this marks the plan
     as trainer-edited.
3. Edits are saved and immediately visible to the customer; this never
   blocks the AI from having already published a plan. If the AI later
   wants to regenerate this plan, the customer is warned first (flow 4)
...
1. Admin logs in (via a seeded demo account with known credentials created
   ...
   - View gym-wide occupancy/equipment-demand dashboard (same aggregate
     data customers see, plus any admin-only detail).
   - Review every medical certificate result — cleared, not_cleared, and
     ...
     wrongly-rejected customer).
   - View each customer's mocked membership status.
```
